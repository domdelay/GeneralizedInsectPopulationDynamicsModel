...
//method that updates the fit fecundity rate values at each associated temperature value based on the current fit decision variable.
void Fitting::setyFit(){
    
    yFit.clear();
    for(int i = 0; i < xData.size(); i++){
        //fecundity rate equation: f(T) = a * exp( -0.5 * pow( (T-b)/c , 2) )
        yFit.push_back(a * exp( -0.5 * pow( (xData.at(i)-b)/c , 2) ) );
    }
}

//method to modify one of the decision variables (a, b, or c) spesified by varID, to improve the fecundity rate equation's fit to the input fecundity values.
void Fitting::fitOneVar(int varID) {
    ...
    for(int i = 0; i < xData.size(); i++){
        double tempNewF = pow(yData.at(i) - (tempa * exp( -0.5 * pow( (xData.at(i)-tempb)/tempc , 2) ) ) , 2);
    ...
    while(std::abs(dif) > 0.0000001){ //0.000001 bassed on how exact we want the value to be
...
//method that fits the decision variable (a, b, c) so that the fecundity rate equation produces very similar values as the input constant temperatures fecundity rates.
