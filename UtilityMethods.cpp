...
std::vector<double> copyDoubleArray(const std::vector<double> toCopy, int len) {
	std::vector<double> copiedArray;
	
	for(int i = 0; i < len; i ++)
		copiedArray.push_back(toCopy.at(i));
...
double sumDoubleArray(const std::vector<double> toSum, int len) {
	double sum = 0;
	for (int i = 0; i < len; i ++)
		sum += toSum.at(i);
	return sum;
